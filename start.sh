...
case $CHOICE in
    1)
        pushd "$SCRIPT_DIR" >/dev/null || { echo "Failure switching to $SCRIPT_DIR"; exit 1; }
        install_script true
        popd >/dev/null || { echo "Failure switching back to original directory"; exit 1; }
        ;;
    2)
        bash "$SCRIPT_DIR/new_lxc.sh"
        ;;
    3)
        exec_in_lxc "install_homebridge.sh"
        ;;
    *)
        echo "Invalid choice"
        ;;
